...
def api_stats():
    """Brain Cluster Stats API (含服务端健康检测)"""
    import sqlite3 as sql, urllib.request
    
    kanban_db = r"D:\brain\memory\kanban.db"
    conn = sql.connect(kanban_db)
    task_stats = dict(conn.execute("SELECT status, COUNT(*) FROM tasks GROUP BY status").fetchall())
    agent_stats = dict(conn.execute("SELECT assignee, COUNT(*) FROM tasks WHERE assignee IS NOT NULL GROUP BY assignee").fetchall())
    conn.close()
    ...
    for name, url in [
        ...
        ("Gateway",  "http://127.0.0.1:18789/api/status"),
        ("Dashboard","http://127.0.0.1:9119/api/health"),
        ("StatsAPI", "http://127.0.0.1:19999/"),
    ]:
        try:
    ...
    return jsonify({
        "kanban": {"by_status": task_stats, "by_agent": agent_stats},
