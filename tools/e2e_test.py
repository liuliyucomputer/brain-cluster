...
# Python 环境
r = subprocess.run([r"E:\Python3134\python.exe", "-c", "import sys; print(sys.version[:10])"], capture_output=True, text=True)
...
# Node.js
r = subprocess.run([r"C:\Users\Administrator\.workbuddy\binaries\node\versions\22.22.2\node.exe", "--version"], capture_output=True, text=True)
check("Node.js 22.22", "22." in r.stdout, r.stdout.strip())
...
# Kanban DB
check("kanban.db 存在", os.path.exists(r"D:\brain\output\memory\kanban.db"),
      f"{os.path.getsize(r'D:\brain\output\memory\kanban.db')} bytes")
...
try:
    ...
    r = subprocess.run([r"E:\Python3134\python.exe", "-c", 
        "import openai; c=openai.OpenAI(base_url='https://tokenshengsheng.com/v1', api_key='sk-xGSsFRUlKUXduzjnwPK4m9J7eNmmVTRwraXROi0dhPiRTvP8'); r=c.chat.completions.create(model='gpt-5.5',messages=[{'role':'user','content':'OK'}],max_tokens=5); print('OK' if r else 'FAIL')"],
        capture_output=True, text=True, env=env, timeout=60)
    check("GPT-5.5 API", "OK" in r.stdout or "data:" in r.stdout, "ccswitch响应正常")
except Exception as e:
